...
class SwapManager(SwapInterface, LagartoServer):
    ...
    def newMoteDetected(self, mote):
        ...
        if self._print_swap == True:
            logging.debug("New mote with address " + str(mote.address) + " : " + mote.definition.product + \
            " (by " + mote.definition.manufacturer + ")")

        self.new_mote_detected(mote)
    ...
    def newEndpointDetected(self, endpoint):
        ...
        """
        if self._print_swap == True:
            logging.debug("New endpoint with Reg ID = " + str(endpoint.getRegId()) + " : " + endpoint.name)

        self.new_endpoint_detected(endpoint)
    ...
    def registerValueChanged(self, register):
        ...
        if self._print_swap == True:
            logging.debug("Register addr= " + str(register.getAddress()) + " id=" + str(register.id) + " changed to " + register.value.toAsciiHex())
        ...
        for endp in register.parameters:
            ...
            if endp.valueChanged:
                if self._print_swap:
                    ...
                    logging.debug(endp.name + " in address " + str(endp.getRegAddress()) + " changed to " + strval)
